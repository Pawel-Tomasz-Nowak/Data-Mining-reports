...
dane <- mutate(dane, Churn = factor(Churn, levels = c("No", "Yes"), labels = c("Loyal", "Disloyal")))

# Pozostałe zmienne kategoryczne też do etykietowania.
```
...
kable(summary.df, digits = 2, caption = "Wskaźniki sumaryczne dla zmiennych ciągłych",
      align = "lr")
```
## Wykresy słupkowe dla wybranych zmiennych kategorycznych
...
Analizując inne cechy, obserwujemy, że największą grupę klientów odchodzących z firmy stanowią osoby, które swoje usługi opłacają, korzystając z czeku elektronicznego; zdecydowały się na kupno swiatłowodu; dostają elektroniczny rachunek za wybrane usługi, lub które nie wybrały usługi wsparcia technicznego.
...
## Dlaczego klienci rezygnują z oferty Telco
