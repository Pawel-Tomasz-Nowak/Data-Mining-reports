...
loadings <- prin.comp$rotation # Wyznacz ładunki składowych głównych.
variances <- (prin.comp$sdev)^2 # Znajdź wariancje składowych głównych.
rotated.df <- as.data.frame(prin.comp$x) # Wyznacz wartości składowych głównych.

# Przekształć rotated.df to postaci długiej (long form)
rotated.df_long <- pivot_longer(rotated.df,cols = everything(), names_to = "Składowa", values_to = "Wartość") 

# Liczba składowych.
n.PCA <- ncol(rotated.df)
