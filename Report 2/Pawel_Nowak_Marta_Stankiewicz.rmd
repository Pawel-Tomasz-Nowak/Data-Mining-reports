...
```{r Sepal Width binning, echo=FALSE, eval=TRUE, results='asis'}
...
```
```{r Petal Width binning, echo=FALSE, eval=TRUE, results='asis'}
...
```{r PCA variances unstd, echo = FALSE, fig.width = 9, fig.height = 6, fig.cap = "Wykresy pudełkowe dla składowych głównych bez zastosowanej standaryzacji"}
...
```{r dissimilarity_matrix, echo=FALSE}
library(cluster)
library(reshape2)
...
# Skalowanie MDS metryczne (k = 2)
fit_cmd <- cmdscale(gower_dist, k = 2, eig = TRUE)
...
mds_df$Survived <- titanic_data$Survived
ggplot(mds_df, aes(x = Dim1, y = Dim2, color = Survived)) +
  geom_point(alpha = 0.8, size = 2) +
  labs(title = "MDS (k = 2): Wykres punktowy wg zmiennej 'Survived'",
       x = "Wymiar 1", y = "Wymiar 2", color = "Survived") +
  theme_minimal()

mds_df$Sex <- titanic_data$Sex
ggplot(mds_df, aes(x = Dim1, y = Dim2, color = Sex)) +
  geom_point(alpha = 0.8, size = 2) +
  labs(title = "MDS (k = 2): Wykres punktowy wg zmiennej 'Sex'",
       x = "Wymiar 1", y = "Wymiar 2", color = "Sex") +
  theme_minimal()

mds_df$Pclass <- titanic_data$Pclass
ggplot(mds_df, aes(x = Dim1, y = Dim2, color = Pclass)) +
  geom_point(alpha = 0.8, size = 2) +
  labs(title = "MDS (k = 2): Wykres punktowy wg zmiennej 'Pclass'",
       x = "Wymiar 1", y = "Wymiar 2", color = "Pclass") +
  theme_minimal()
