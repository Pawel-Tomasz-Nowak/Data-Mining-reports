...
```{r mds_2, fig.cap = "Diagram Sheparda po zastosowaniu MDS dla k = 2",fig.width = 5, fig.height = 5}
mds_k(2)
```

```{r mds_3, fig.cap = "Diagram Sheparda po zastosowaniu MDS dla k = 2",fig.height = 5, fig.width = 6}
mds_k(3)
```

Analiza diagramu Sheparda wskazuje, że skalowanie klasyczne w przestrzeni dwuwymiarowej charakteryzuje się niską jakością odwzorowania – obserwacje wykazują istotne odchylenia od linii idealnej, co świadczy o niewystarczającej dokładności odwzorowania struktur odległościowych. Zwiększenie liczby wymiarów do trzech prowadzi do zauważalnej poprawy – punkty na wykresie są znacznie bliższe linii odniesienia, co potwierdza wyższą trafność rekonstrukcji oryginalnych relacji odległościowych w przestrzeni trójwymiarowej.
