...
```{r violin boxplots, fig.width = 9, fig.height = 8, fig.cap = "Wykresy skrzypcowo-pudełkowe dla zmiennych ciągłych"}
...
```{r discretization method comparison}
...
evaluate_method <- function(df, var){
  ...
  results <- t(apply(df.discr_vals, 1, do_class_match)) %>% 
...
```{r Sepal Width binning, eval=TRUE, results='asis'}

results <- t(data.frame(evaluate_method(iris_df, "Sepal.Width")))

colnames(results) <- c("Równa częstotliwość",
                            "Równa szerokości",
                            "K-średnie",
                            "Stałe granice")


tab1 <- xtable(results, digits = 3, row.names = FALSE, caption = "Ocena przedziałowania zmiennej Sepal Width", label = "tab:Sepal.Width.Binning")
print(tab1, type = "latex", table.placement = "H", comment=FALSE, include.rownames = FALSE)

```
```{r Petal Width binning, eval=TRUE, results='asis'}

results <- t(data.frame(evaluate_method(iris_df, "Petal.Width")))
...
tab2 <- xtable(results, digits = 3, row.names = FALSE, caption = "Ocena przedziałowania zmiennej Petal Width", label = "tab:Petal.Width.Binning")
print(tab2, type = "latex", table.placement = "H", comment=FALSE, include.rownames = FALSE)

```

### Wnioski dotyczące skuteczności przedziałowania
Z tabel \ref{tab:Sepal.Width.Binning} oraz \ref{tab:Petal.Width.Binning} możemy wywnioskować, że w obu przypadkach największą skutecznością charakteryzuje się metoda dyskretyzacji oparta na **algorytmie K-średnich**. Z kolei najgorszą skuteczność przedziałowania obserwujemy dla metody opartej na **stałych granicach** przedziału.
...
```{r variance_comparison_before,fig.cap="Wykresy pudelkowe zmiennych ciągłych przed zastosowaniem standaryzacją",fig.width = 9, fig.height  = 8}
...
```{r variance_comparison_after, fig.cap="Wykresy pudelkowe zmiennych ciągłych po zastosowaniu standaryzacji", fig.width = 9, fig.height  = 8}
...
```{r corr matrix, fig.cap = "Macierz korelacji dla zmiennych ciągłych"}
corr_matrix <- abs(cor(x = CQL_df[num.cols_CQL], use = "complete.obs"))

old_par <- par(no.readonly = TRUE)
par(mar = c(1, 1, 5, 1))  # Dół, lewo, góra, prawo

corrplot(corr_matrix, method = "square", type = "full", tl.cex = 0.8,
         tl.pos = FALSE, col.lim = c(0, 1))

par(old_par)  # Przywróć ustawienia
```
...
```{r principal components computation}
...
```{r PC variance visualization, fig.height =5, fig.width = 6, fig.cap = "Wykresy pudełkowe dla składowych głównych"}
...
```{r variances explained plot, fig.height=8, fig.width = 9, fig.cap = "Porównanie udziału wariancji wyjaśnianej przez poszczególne składowe główne"}
...
```{r cum variances explained plot, fig.height=5, fig.width = 6, fig.cap = "Skumulowana wariancja wyjaśniana przez kolejne składowe główne"}
...
```{r PCA visualization, fig.height = 8, fig.width = 9, fig.cap = "Porównanie udziału wariancji wyjaśnianej przez poszczególne składowe główne bez standaryzacji"}
...
```{r PCA variances unstd, fig.width = 6, fig.height = 5, fig.cap = "Wykresy pudełkowe dla składowych głównych bez zastosowanej standaryzacji"}
...
```{r cum variances explained plot unstd, fig.height = 5, fig.width = 6,fig.cap = "Skumulowana wariancja wyjaśniana przez kolejne składowe główne"}
...
identifying <- c("PassengerId", "Name", "Ticket", "Cabin")
titanic_data <- dplyr::select(titanic_data, -dplyr::all_of(intersect(names(titanic_data), identifying)))
...
gower_dist <- daisy(titanic_dissim, metric = "gower")

# konwersja do zwykłej macierzy
...
fit_cmd <- cmdscale(gower_dist, k = k, eig = TRUE)


# Obliczenie danych do wykresu Sheparda
mds_distances <- dist(fit_cmd$points)

plot(as.vector(gower_dist), as.vector(mds_distances),
     main = paste("Diagram Sheparda (dla wymiaru", k, ")"),
...
```{r mds_3, fig.cap = "Diagram Sheparda po zastosowaniu MDS dla k = 3",fig.height = 5, fig.width = 6}
mds_k(3)
```
W celu oceny jakości odwzorowania odległości między obiektami w niżowymiarowych przestrzeniach, zastosowano diagram Sheparda. W przypadku skalowania klasycznego (classical MDS) do przestrzeni trójwymiarowej zaobserwowano bardzo dobrą zgodność między odległościami oryginalnymi a odległościami w zredukowanej przestrzeni – punkty diagramu układają się blisko linii odniesieniam, co świadczy o wysokiej dokładności rekonstrukcji i zachowaniu struktury danych.
Natomiast projekcja do przestrzeni dwuwymiarowej wykazała istotny spadek jakości odwzorowania. Punkty na diagramie Sheparda w tym przypadku tworzą rozproszoną chmurę, oddaloną od linii idealnej, co wskazuje na znaczne zniekształcenia oryginalnych relacji dystansowych. Takie wyniki sugerują, że reprezentacja trójwymiarowa znacznie lepiej oddaje strukturę danych niż reprezentacja dwuwymiarowa, i powinna być preferowana w dalszej analizie, jeśli interpretowalność nie wymaga dalszego uproszczenia.


## Wizualizacja danych po zastosowaniu skalowania wielowymiarowego
```{r no_group_mds_scatterplot, fig.cap = "Wykres rozrzutu po zastosowaniu MDS dla k = 2", fig.height = 5,fig.width = 6}
library(glue)
  
fit_cmd <- cmdscale(gower_dist, k = 2)
mds_df <- as.data.frame(fit_cmd)
colnames(mds_df) <- c("Dim1", "Dim2")

ggplot(mds_df, aes(x = Dim1, y = Dim2)) +
  geom_point(color = "black", alpha = 0.6) +
  labs(title = "MDS (k = 2): Wykres punktowy bez grupowania",
       x = "Wymiar 1", y = "Wymiar 2") +
  theme_minimal()


grouped_mds_scatterplot <- function(var){
mds_df[[var]] <- titanic_data[[var]]

plot <- ggplot(mds_df, aes(x = Dim1, y = Dim2, color = !!sym(var))) +
  geom_point(alpha = 0.8, size = 2) +
  labs(
    title = "MDS (k = 2): Wykres rozrzutu wg zmiennej Survived",
    x = "Wymiar 1",
    y = "Wymiar 2",
    color = var
  )+
...
if (var == "Survived"){
  plot <- plot + scale_color_manual(values = c("0" = "red", "1" = "green"),
...
```{r survived_mds_scatterplot, fig.cap = "Wykres rozrzutu po zastosowaniu MDS dla k = 2 z grupowaniem według zmiennej 'Survived'", fig.height = 5, fig.width = 6}
grouped_mds_scatterplot("Survived")
```

```{r pclass_mds_scatterplot, fig.cap = "Wykres rozrzutu po zastosowaniu MDS dla k = 2 z grupowaniem według zmiennej 'Pclass'", fig.height = 5, fig.width = 6}
grouped_mds_scatterplot("Pclass")
```

```{r sex_mds_scatterplot, fig.cap = "Wykres rozrzutu po zastosowaniu MDS dla k = 2 z grupowaniem według zmiennej 'Sex'", fig.height = 5, fig.width = 6}
grouped_mds_scatterplot("Sex")
```

Na wykresie \ref{fig:survived_mds_scatterplot} widoczny jest wyraźny podział danych na dwa skupiska. Po oznaczeniu przynależności poszczególnych obserwacji do grup odpowiadających wartościom zmiennej **Survived**, można zauważyć, że w górnym skupisku dominują osoby, które przeżyły (**Survived** == 1), natomiast w dolnym – osoby, które nie przeżyły (**Survived** == 0). Na wykresie można dostrzec kilka punktów oddalonych od głównych skupisk, np. pojedyncze punkty znajdujące się po lewej stronie wykresu lub w dolnej części, z dala od skoncentrowanych grup. Są to obserwacje odstające, które mogą reprezentować osoby o nietypowym zestawie cech (np. bardzo młody wiek, nietypowa kombinacja klasy i płci) lub wskazywać na błędy danych lub wyjątkowe przypadki. Cechy użyte do analizy MDS w pewnym stopniu odzwierciedlają różnice związane z przeżyciem, choć nie jest to rozdział idealny – kolory są częściowo wymieszane w niektórych skupiskach.
Po uwzględnieniu podziału według płci \ref{fig:sex_mds_scatterplot} widać, że wśród osób ocalałych największą grupę stanowią kobiety. Dalsza analiza z uwzględnieniem klasy podróży \ref{fig:pclass_mds_scatterplot} (zmienna **Pclass**) pokazuje, że wśród kobiet, które przeżyły, zdecydowaną większość stanowią pasażerki podróżujące pierwszą klasą.
W przypadku mężczyzn natomiast klasa podróży nie miała istotnego wpływu na przeżywalność – liczba ocalałych była zbliżona niezależnie od klasy.
