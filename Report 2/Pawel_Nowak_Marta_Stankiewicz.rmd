...
evaluate_method <- function(df, var){
  ...
  result_match <- t(apply(df.discr_vals, 1, do_class_match)) %>% 
    apply(2, function(x){ sum(x)/nrow(df.discr_vals)}) %>%
    round(2)
  
  # The table with the metrics of accuracy of discretization.
  acc_table <- kable(t(data.frame(Skutecznosc = result_match)), 
                col.names =c("equal_frequency", "equal_width", "cluster_based", "fixed-bounds"), 
                row.names = FALSE, 
                caption = paste("Skuteczność wybranych metod dyskretyzacji dla zmiennej", var))
